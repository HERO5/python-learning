...
def train(params=None):
    ...
    if not tf.gfile.Exists('../data/mnist/model/'):
        tf.gfile.MakeDirs('../data/mnist/model/')
    saver = tf.train.Saver()  # 保存模型 实例化
    saver.save(sess, '../data/mnist/model/my_model.ckpt')

def read_model():
    #打印参数
    with tf.Session() as sess:
        ckpt = tf.train.get_checkpoint_state('../data/mnist/model/')
